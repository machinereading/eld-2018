    ...
    def train(self, org_train_dataset, org_dev_datasets, config):
        ...
        for e in range(config['n_epochs']):
            ...
            for dc, batch in enumerate(train_dataset):  # each document is a minibatch
                ...
                self.model.s_ltoken_ids = Variable(torch.LongTensor(s_ltoken_ids).to(device))
                self.model.s_ltoken_mask = Variable(torch.FloatTensor(s_ltoken_mask).to(device))
                self.model.s_rtoken_ids = Variable(torch.LongTensor(s_rtoken_ids).to(device))
                self.model.s_rtoken_mask = Variable(torch.FloatTensor(s_rtoken_mask).to(device))
                self.model.s_mtoken_ids = Variable(torch.LongTensor(s_mtoken_ids).to(device))
                self.model.s_mtoken_mask = Variable(torch.FloatTensor(s_mtoken_mask).to(device))
    ...
    def predict(self, data):
        ...
        for batch in data:  # each document is a minibatch
            ...
            for i, score in enumerate(scores):
                ...
                non_inf_list = list(filter(lambda x: x != float('-inf'), scores[i]))
                mean = np.mean(non_inf_list)
                processed_scores.append(((scores[i] - mean) * len(non_inf_list) * 10).tolist())
                
            e_x = np.exp(processed_scores - np.amax(processed_scores, axis=1, keepdims=True))
            ...
            for dname, entity, pred_score, pred_confidence in zip(doc_names, pred_entities, pred_scores, pred_confidences):
                predictions[dname].append({'pred': (entity, 0.), 'score': pred_score, 'confidence': pred_confidence})
        return predictions
